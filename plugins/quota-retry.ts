...
//
// 更新机制(见 shared/sync.ts, 配置 ~/.config/opencode/buraunkan-kobo.jsonc):
//   opencode 对 git 插件只安装一次(缓存 ~/.cache/opencode/packages/), 之后不再拉取。
//   三个插件共享一份节流状态(默认 24h 一次), 窗口内第一个加载的用 git ls-remote
...
function loadConfig(projectDir: string): PluginConfig {
  ...
  for (const file of candidates) {
    const parsed = loadJsonc<PluginConfig>(file, "quota-retry")
...
export const quotaRetry = async (input: { directory?: string; client?: any }) => {
  if (!gateActive("quota-retry")) return {}
  ...
  // 自更新: 节流 + opencode 原生补装(见 shared/sync.ts, 配置在 buraunkan-kobo.jsonc)
  void maybeSync(toast)
  ...
  return {
    ...
    "command.execute.before": async (input: { command: string; sessionID?: string }, _output: unknown) => {
      ...
      await replyLocal(toastClient, input.sessionID as string, text, "__QUOTA_RETRY_HANDLED__")
    },
    tool: {
      quota_retry_status: {
        description:
